...
class _VotingStoriesState extends State<VotingStories> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    final activeStories = widget.room.stories.where((t) => [StoryStatusEnum.newStory, StoryStatusEnum.voting].contains(t.status)).toList();
    final completedStories = widget.room.stories.where((t) => t.status == StoryStatusEnum.voted).toList();
    return Container(
                  ...
                  child: Column(
                    children:
                        activeStories
                            .map(
                              (t) =>
                                  Container(padding: EdgeInsets.symmetric(horizontal: 16, vertical: 10), alignment: Alignment.centerLeft, height: 50, child: Text(t.description)),
                            )
                            .toList(),
                  ),
                ),
